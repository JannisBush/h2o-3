...
def mojo_predict_csv_test(target_dir):
    ...
    assert abs(binary_prediction_0 - mojo_prediction_0) < 1e-8, "expected predictions to be the same for binary and MOJO model for Binomial - p0"
    assert abs(binary_prediction_1 - mojo_prediction_1) < 1e-8, "expected predictions to be the same for binary and MOJO model for Binomial - p1"
    ...
    assert abs(multinomial_prediction_1 - mojo_prediction_1) < 1e-8, "expected predictions to be the same for binary and MOJO model for Multinomial - p0"
    assert abs(multinomial_prediction_2 - mojo_prediction_2) < 1e-8, "expected predictions to be the same for binary and MOJO model for Multinomial - p1"
    assert abs(multinomial_prediction_3 - mojo_prediction_3) < 1e-8, "expected predictions to be the same for binary and MOJO model for Multinomial - p2"
...
def mojo_predict_pandas_test(sandbox_dir):
    ...
    assert abs(h2o_prediction[0,1] - mojo_prediction['p0'].iloc[0]) < 1e-8, "expected predictions to be the same for binary and MOJO model - p0"
    assert abs(h2o_prediction[0,2] - mojo_prediction['p1'].iloc[0]) < 1e-8, "expected predictions to be the same for binary and MOJO model - p0"
